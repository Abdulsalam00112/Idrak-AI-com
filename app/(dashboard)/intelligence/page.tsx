...
type TopicRecord = { topicId: string; name: string; subject: string; performance: number; questionsAnswered: number; examWeight: number; priority: 'critical' | 'high' | 'maintain' | 'medium'; trend: string; subtopics: number };
...
export default function IntelligencePage() {
  ...
  useEffect(() => {
    fetch('/api/analytics', { credentials: 'include' })
      .then(async (response) => { const payload = await response.json(); if (!response.ok) throw new Error(payload.error?.message || 'Unable to load topic intelligence.'); return payload.data?.topics ?? []; })
      .then((rows) => setTopicData(rows.map((topic: { topic_id: string; topic_name?: string; accuracy: number; questions_attempted: number }) => ({ topicId: topic.topic_id, name: topic.topic_name || 'Uncategorized topic', subject: 'Your practice', performance: Number(topic.accuracy || 0), questionsAnswered: Number(topic.questions_attempted || 0), examWeight: 0, priority: Number(topic.accuracy || 0) < 50 ? 'critical' : Number(topic.accuracy || 0) < 70 ? 'high' : Number(topic.accuracy || 0) >= 80 ? 'maintain' : 'medium', trend: 'stable', subtopics: 0 }))))
      .catch((reason: Error) => setError(reason.message));
  }, []);

  const TOPIC_INTELLIGENCE = topicData;
  const WEEKLY_PLAN: { day: string; items: { title: string; subject: string; duration: number }[] }[] = [];
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-ink-soft mt-1 max-w-2xl">
          Idrak cross-references your practice performance with topic data to show you exactly where to focus.
        </p>
      </div>
