...
const SECONDARY_ITEMS: NavItem[] = [
  ...
  { href: '/api-portal', label: 'API & Developers', icon: KeyRound },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/session', { credentials: 'include' })
      ...
      .then((payload) => {
        if (!active) return;
        if (!payload.data?.user) { router.replace('/login'); return; }
        // A user who signed up but never finished onboarding shouldn't land
        // on the dashboard (or any other in-app page) just by visiting the
        // URL directly — send them back to pick up where they left off.
        if (payload.data.user.onboardingComplete === false) { router.replace('/onboarding'); return; }
          ...
          <div className="flex items-center gap-2">
            ...
            {user && (
              <div className="hidden sm:block">
                <Badge tone="coral" size="md" dot>{(getExamById(user.examId)?.shortName) || 'Idrak'} prep</Badge>
              </div>
            )}
          </div>
        </header>
